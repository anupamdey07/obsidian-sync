...
* **System 2 (The Hardened Conscience):** A local SLM (Small Language Model, like Phi-4-mini or Gemma 3) running as a Kubernetes Deployment (`policy-engine`). It acts as the ultimate deterministic bouncer — physically isolating the Left Brain from the Right Brain, reasoning over every `/action` to validate safety and hardware constraints before execution. Backed by a formally verified seL4 Microkernel (Type-1 Hypervisor) where hardware-level isolation is required.
...
### Tri-Brain to System Architecture Mapping
...
### 3.1. Talos Extension Layer (Host)
...
### 3.2. Kubernetes System Pods (Fabric OS Core)
...
### 3.3. LLM Workload Pods (AI Layer)
...
## 4. EDGE-DISTRIBUTED TOPOLOGY & UNIVERSAL COMPUTE
...
## 5. HARDWARE & INFRASTRUCTURE SPECIFICATION (The Core Node)
...
### 5.1. Asus GX10 vs. NVIDIA Spark DGX
...
### 5.2. Storage Constraints & The 4TB Requirement
...
## 6. COLD START: SELF-DISCOVERY PROTOCOL
...
The key design principle: **host layer for reachability, safety, and recovery; cluster layer for product behavior; workload layer for models.**
